...
export function useCreatePerson() {
  ...
  return useMutation({ mutationFn: (data: Partial<Person>) => personsApi.create(data), onSuccess: () => qc.invalidateQueries({ queryKey: personKeys.all }) })
}

export function useUpdatePerson(id: number) {
  const qc = useQueryClient()
  return useMutation({ mutationFn: (data: Partial<Person>) => personsApi.update(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: personKeys.all }); qc.invalidateQueries({ queryKey: personKeys.detail(id) }) } })
}
