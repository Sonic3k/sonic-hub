...
export const personKeys = { all: ['persons'] as const, detail: (id: string) => ['persons', id] as const }

export function usePersons() { return useQuery({ queryKey: personKeys.all, queryFn: personsApi.getAll }) }
export function usePerson(id: string) { return useQuery({ queryKey: personKeys.detail(id), queryFn: () => personsApi.getById(id), enabled: !!id }) }
...
export function useDeletePerson() {
  const qc = useQueryClient()
  return useMutation({ mutationFn: (id: string) => personsApi.delete(id), onSuccess: () => qc.invalidateQueries({ queryKey: personKeys.all }) })
}
