  ...
  return (
    <>
      ...
      <aside className={clsx(
        'fixed z-50 top-0 left-0 h-screen w-64 md:w-56 bg-[#0f1117] flex flex-col transition-transform duration-200 ease-in-out',
        ...
        <div className="px-5 py-5 border-b border-[#1e2130] flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-[#8b92a8] hover:text-white md:hidden p-1.5 -mr-1">
            <X size={18} />
          </button>
        </div>
          ...
          {navItems.map(({ to, label, icon: Icon, end }) => (
            <NavLink key={to} to={to} end={end} onClick={onClose}
              className={({ isActive }) => clsx(
                ...
                isActive ? 'bg-[#1e2130] text-white' : 'text-[#8b92a8] hover:bg-[#1a1d27] hover:text-[#c4c9d9] active:bg-[#1e2130]'
              )}>
              <Icon size={17} />{label}
            </NavLink>
          ))}
